...
async function createNotification(app, payload) {
  const { userId, title, message, type, link, dedupeKey, apptId, text } = payload || {};
...
async function notifyChat(app, msg) {
  try {
    ...
    const type = 'chat';
    const link = toDoctor ? '/doctor/dashboard' : '/appointments?alertChat=1';
    await createNotification(app, { userId, title, message, type, link, apptId, text });
  } catch (_) {}
}
